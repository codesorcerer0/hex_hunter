...
generated_keys = Value('i', 0)
found_addresses = Value('i', 0)
lock = Lock()
...
def worker(database):
    ...
    while True:
        ...
        with lock:
            generated_keys.value += 1
            found_addresses.value += local_found

        if local_generated % 5000 == 0:
            elapsed_time = time.time() - start_time
            with lock:
                total_keys = generated_keys.value
                total_found = found_addresses.value
            keys_per_second = total_keys / elapsed_time

            print("=" * 60)
            print(f"{Fore.LIGHTYELLOW_EX}Keys Generated: {total_keys} | Found: {total_found} | Speed: {keys_per_second:.2f} keys/s")
...
def main():
    ...
    if choice == '1':
        ...
        if any(addr in database for addr in [uncompressed_address, compressed_address, bech32_address, p2sh_address]):
            print(f"{Fore.LIGHTGREEN_EX}Match Found!")
            # Save match to HEX_HUNTER.txt
            with open('found.txt', 'a') as BTC:
                BTC.write(f"Private Key (Hex): {private_key}\n")
                BTC.write(f"Uncompressed Address (P2PKH): {uncompressed_address}\n")
                BTC.write(f"Compressed Address (P2PKH): {compressed_address}\n")
